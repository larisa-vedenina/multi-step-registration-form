...
Здесь важен не только внешний вид формы, но и цельный пользовательский сценарий:

- сначала пользователь заполняет основные поля
- потом проходит проверку через captcha
- дальше подтверждает действие по SMS-коду
- после этого попадает на отдельный экран с выбором даты и времени
