...
Изначально форма появилась в рамках рабочей задачи, а затем была адаптирована для портфолио. В проекте мной были проработаны дизайн-макет, пользовательский сценарий, верстка интерфейса и логика шагов на `vanilla JavaScript`.
...
- форма регистрации с пошаговым сценарием
- проверка полей в реальном времени
- captcha с генерацией кода на клиенте
- SMS-шаг с 4-значным кодом
- выбор кода страны для телефона
- отдельная страница после регистрации
- адаптация под мобильные экраны
- аккуратная базовая доступность: `label`, `aria-live`, `aria-invalid`, корректная блокировка кнопки
